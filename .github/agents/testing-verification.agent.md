...
- Canonical normalization gate: search all validators, handlers, mapping helpers, and commands for equivalent numeric/date/enum algorithms; identify the single factory/value-object owner; run direct owner tests and request-validator tests; refuse sign-off when a command-local copy remains or owner boundary coverage is missing.
- Reference-data contract gate: require direct query-handler and provider-backed tests for normalized found, unknown/malformed not-found, and inactive-state responses. Inspect downstream consumers for public-contract usage, nullable failure semantics, and absence of direct owning-feature DbContext access.
- Provider-backed suite gate: for every persistence-bearing feature, inspect the feature test project for `Microsoft.EntityFrameworkCore.SqlServer`, a unique-database harness, migration setup, fresh-context persistence assertions, and best-effort cleanup. Execute the suite and report its test count; refuse sign-off when only InMemory tests are present.
