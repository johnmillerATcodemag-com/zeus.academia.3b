...
- A feature that advertises validation responses must map actual validation/normalization exceptions (`ArgumentException`, `ArgumentOutOfRangeException`, etc.) to `Results.ValidationProblem(...)` or equivalent, and tests must cover invalid numeric/range input.
