...
- MUST run a runtime-readiness audit before opening PR:
  - Every route group or endpoint map must be mounted in the application host before merge.
  - Every feature-local `DbContext` must include migration artifacts or an explicit migration strategy before enabling startup migration.
  - Validation rules must operate on the same normalized value as the domain model (for example `Trim()`/`ToUpperInvariant()` before length and allowed-value checks).
  - Invalid input must be translated to a client-safe validation response at the API boundary; do not allow `ArgumentException` or domain validation errors to bubble into HTTP 500 responses.
